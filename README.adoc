...
DEPLOYED_VERSION_CMD=$(CF_COLOR=false cf apps | grep 'cities-uniquetoken-' | cut -d" " -f1)
DEPLOYED_VERSION="$DEPLOYED_VERSION_CMD"
ROUTE_VERSION=$(echo "${BUILD_VERSION}" | cut -d"." -f1-3 | tr '.' '-')
echo "Deployed Version: $DEPLOYED_VERSION"
echo "Route Version: $ROUTE_VERSION"

cf push "cities-uniquetoken-$BUILD_VERSION" -i 1 -m 512M -n "cities-uniquetoken-$ROUTE_VERSION" -d cfapps.io -p
artifacts/citytest-${BUILD_VERSION}.jar --no-manifest
cf map-route "cities-uniquetoken-${BUILD_VERSION}" cfapps.io -n cities-uniquetoken
cf scale cities-uniquetoken-${BUILD_VERSION} -i 2
if [ ! -z "$DEPLOYED_VERSION" -a "$DEPLOYED_VERSION" != " " -a "$DEPLOYED_VERSION" != "cities-uniquetoken-${BUILD_VERSION}" ]; then
  echo "Performing zero-downtime cutover to $BUILD_VERSION"
  while read line
  do
    if [ ! -z "$line" -a "$line" != " " -a "$line" != "cities-uniquetoken-${BUILD_VERSION}" ]; then
